...
final class Ikonic_Contact_SMTP_Form {
    ...
    private const RATE_LIMIT_PREFIX = 'icsf_rate_limit_';
...
CSS;
    ...
    public function render_forms_page(): void {
        ...
        $active_form = wp_parse_args($active_form, $this->default_form());
        ?>
        <div class="wrap">
            ...
            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                ...
                <table class="form-table">
                    ...
                    <tr><th><?php esc_html_e('Enable Honeypot Anti-Spam', 'ikonic-contact-smtp-form'); ?></th><td><label><input type="checkbox" name="enable_honeypot" value="1" <?php checked(!empty($active_form['enable_honeypot'])); ?> /> <?php esc_html_e('Enable hidden bot trap field', 'ikonic-contact-smtp-form'); ?></label></td></tr>
                    <tr><th><?php esc_html_e('Store Entries in Analytics', 'ikonic-contact-smtp-form'); ?></th><td><label><input type="checkbox" name="store_entries" value="1" <?php checked(!empty($active_form['store_entries'])); ?> /> <?php esc_html_e('Save submissions in plugin analytics', 'ikonic-contact-smtp-form'); ?></label></td></tr>
                    <tr><th><?php esc_html_e('Webhook URL (optional)', 'ikonic-contact-smtp-form'); ?></th><td><input type="url" name="webhook_url" class="regular-text" value="<?php echo esc_attr($active_form['webhook_url']); ?>" placeholder="https://example.com/webhook" /></td></tr>
                </table>
    ...
    public function render_analytics_page(): void {
        ...
        foreach ($logs as $log) {
            ...
            if ($status === 'success') { $success_count++; } else { $error_count++; }
            $ip = $log['ip'] ?? '';
            if ($ip !== '') { $by_ip[$ip] = ($by_ip[$ip] ?? 0) + 1; }
        ...
        <div class="wrap">
            ...
            <p><strong><?php esc_html_e('Successful deliveries:', 'ikonic-contact-smtp-form'); ?></strong> <?php echo esc_html((string) $success_count); ?> | <strong><?php esc_html_e('Failed/blocked:', 'ikonic-contact-smtp-form'); ?></strong> <?php echo esc_html((string) $error_count); ?></p>
            ...
            <h2><?php esc_html_e('Top Visitor IPs', 'ikonic-contact-smtp-form'); ?></h2>
            <table class="widefat striped" style="max-width:800px;">
                <thead><tr><th><?php esc_html_e('IP', 'ikonic-contact-smtp-form'); ?></th><th><?php esc_html_e('Submissions', 'ikonic-contact-smtp-form'); ?></th></tr></thead>
                <tbody>
                <?php if (empty($by_ip)) : ?>
                    <tr><td colspan="2"><?php esc_html_e('No IP data yet.', 'ikonic-contact-smtp-form'); ?></td></tr>
                <?php else : foreach (array_slice($by_ip, 0, 10, true) as $ip => $count) : ?>
                    <tr><td><?php echo esc_html($ip); ?></td><td><?php echo esc_html((string) $count); ?></td></tr>
                <?php endforeach; endif; ?>
                </tbody>
            </table>
    ...
    public function render_contact_form_shortcode(array $atts): string {
        ...
        <form method="post" class="icsf-contact-form">
            ...
            <?php if (!empty($form['enable_honeypot'])) : ?>
                <p class="icsf-field-full" style="position:absolute;left:-9999px;opacity:0;pointer-events:none;" aria-hidden="true">
                    <label><?php esc_html_e('Leave this field empty', 'ikonic-contact-smtp-form'); ?></label>
                    <input type="text" name="icsf_hp" value="" tabindex="-1" autocomplete="off" />
                </p>
            <?php endif; ?>
    ...
    public function handle_form_submission(): void {
        ...
        if (!empty($form['enable_honeypot']) && !empty($_POST['icsf_hp'])) {
            $this->store_log([
                'form_id' => $form['id'],
        ...
        if (!empty($form['webhook_url']) && $sent) {
            wp_remote_post($form['webhook_url'], [
                'timeout' => 10,
    ...
    private function resolve_form_by_id(string $id): array {
        ...
        if ($id !== '' && isset($forms[$id])) {
            return wp_parse_args($forms[$id], $this->default_form());
        }
        if (!empty($forms)) {
            return wp_parse_args((array) reset($forms), $this->default_form());
        }
        return [
            'id' => 'default-contact',
